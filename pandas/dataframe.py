...
)  # here the index is given as a list of strings ['a', 'b', 'c', 'd', 'e'] which will be used as the index for the DataFrame.
...
new_rows = pd.DataFrame(
    {"Name": ["Frank", "Grace", "Hank"], "Age": [50, 55, 60]}
)  # here we create a new row as a dictionary where the keys are the column names and the values are the data for each column.
...
df.index = [
    ...
    "g",
    "h",
]  # we we dont give this then old data have index a to e and new data have index 0 to 2 which is not good so we need to give the index for new data also.

# print(df)


# to add the new column in dataframe we can do it like this
df["Jobs"] = [
    "Engineer",
    "Teacher",
    "Doctor",
